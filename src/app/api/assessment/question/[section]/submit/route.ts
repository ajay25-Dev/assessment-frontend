...
const allowedSections = new Set(["dsa", "sql", "oops", "mcq"]);
...
export async function POST(
  ...
  if (!allowedSections.has(normalizedSection)) {
    return NextResponse.json({ message: "Invalid assessment section" }, { status: 400 });
  }
  ...
  const response = await fetch(`${backendBaseUrl()}/assessment/${normalizedSection}/submit`, {
    method: "POST",
  ...
  const payload = await response.json().catch(() => ({
    message: `Question submission failed with status ${response.status}`,
  }));
